...
/* colour: 7 bins over the metric's range; dir=-1 means low value reads as severe */
function binOf(d){
  const vals = C.map(metric.get);
  const min = Math.min(...vals), max = Math.max(...vals);
  let t = (metric.get(d) - min) / (max - min || 1);
  if (metric.dir === -1) t = 1 - t;
